...
define(['./utils', './CapServer'], function(utils, CapServer) {
  ...
  var now = (
    ('now' in Date) ? Date.now : function() { return Number(new Date()); } );

  // TODO(mzero): should use the version from util, but that version returns
  // a nullUUID, not null on failure
