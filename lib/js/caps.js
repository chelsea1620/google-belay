...
var CAP_EXPORTS = (function() {
  ...
  var CapServer = function(instanceId, snapshot) {
    ...
    if (instanceId !== undefined) {
      if (typeof(instanceId) !== 'string' || !isUUIDv4(instanceId)) {
        throw new TypeError(
          'CapServer: bad instance identifier: must be a v4 UUID');
      }
    }
  ...
  /**
   * Builds an invokeable wrapper around the provided item for use
   * as part of a Capability object. Typically used when reconstructing
   * capability handlers in the function provided to setResolver().
   *
   * Legal item arguments are one of the following:
   ...
   * form function(key) which will return a reconstructed capability
   * handler, created by invoking build() with the handling item.
   */
  CapServer.prototype.setReviver = function(r) { this.reviver = r; };

  /**
   * Returns a snapshot of the internal state of this capability
   * server as a JSON serialized value, for storage and use in 
   * reconstructing the capability server in a future session.
   */
