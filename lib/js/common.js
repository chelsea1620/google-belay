...
var common = {
  makeCapDraggable: function(capServer, dirty) {
    return function(node, rc, generator, imgurl) { 
      ...
      node.bind('dragstart', function(evt) {
        ...
        var saveGen = capServer.grant(function(rc) {
    ...
    return function(node, rc, accept) {
      ...
      node[0].addEventListener('drop', function(evt) {
        ...
        try {
          ...
          elt = $(html);
          var jsonData = elt[1].getAttribute('data');
          ...
          console.log('ignoring dropped nonsense, exn: ', e,
